...
export default function Product({ path, listproduct, brumb }) {
  ...
  useEffect(() => {
    const product = () => {
      getProduct(
        {},
        (res) => {
          const data = res.data.data.filter((item) => item.category === brumb);
          setProduct(data);
        },
        (err) => {
          console.log(err);
  ...
  if (valuePrice === 2000000) {
    productss = products.filter((product) => parseInt(product.price) > 1000000);
  } else if (valuePrice === 0) {
    productss = products;
  } else {
    productss = products.filter(
      (product) => parseInt(product.price) < valuePrice
    );
  }

  if (isKeyProduct) {
    productss = products.filter((product) =>
      product.title.toLowerCase().includes(keyProduct.trim().toLowerCase())
    );
  }

  const displayProduct = productss
    .slice(pageVisit, pageVisit + productPage)
    .map((product, index) => {
      ...
      <filterProduct.Provider value={{ changPrice }}>
        ...
        <div className="container content">
          ...
          <div className="content__list">
            ...
            <div className="content__product">{displayProduct}</div>
            {productss.length !== 0 ? (
